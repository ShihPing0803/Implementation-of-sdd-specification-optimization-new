...
from service import add_record, import_records, update_record, delete_record, total_records, find_record_by_id, summary_record
from storage import load_records
from display import print_records, choose_type, print_summary
...
def main():
    ...
    if args.command == "add":
        ...
        if category and category.strip().lower() == "uncategorized":
            category = "-"
        
        result = load_records(args.type, category, args.startAt, args.endAt, args.sortBy, args.order)
        ...
        update_category = input(f"Please input a new category or press Enter to skip (current: {record.get('category', 'uncategorized')}): ").strip()
        if update_category:
        ...
        result = summary_record(args.type)
        print_summary(result)
        return
    elif args.command == "import":
        message = import_records(args.file)
        print(message)
